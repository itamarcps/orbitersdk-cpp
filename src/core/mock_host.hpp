...
public:
    ...
    evmc_vm* vm_; // EVMC VM instance so we can use it to execute code on call()
...
public:
    ...
    Result call(const evmc_message& msg) noexcept override
    {
        ...
        BytesArr<20> addressFrom;
        BytesArr<20> addressTo;
        BytesArr<20> contractAddrCode;
        Bytes data;
        std::copy(msg.sender.bytes, msg.sender.bytes + 20, addressFrom.begin());
        std::copy(msg.recipient.bytes, msg.recipient.bytes + 20, addressTo.begin());
        std::copy(msg.input_data, msg.input_data + msg.input_size, std::back_inserter(data));
        std::copy(msg.code_address.bytes, msg.code_address.bytes + 20, contractAddrCode.begin());
        std::cout << "From: " << Hex::fromBytes(addressFrom) << std::endl;
        std::cout << "To: " << Hex::fromBytes(addressTo) << std::endl;
        std::cout << "Data: " << Hex::fromBytes(data) << std::endl;
        std::cout << "Contract: " << Hex::fromBytes(contractAddrCode) << std::endl;
